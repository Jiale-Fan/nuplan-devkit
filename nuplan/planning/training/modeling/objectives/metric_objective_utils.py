...
def get_metric_scores(trajectories: Trajectory, scenarios: ScenarioListType, metric: MetricBase) -> Tensor:
    trajectories = trajectories.unpack()
    score_list = [ get_single_metric_score(trajectory, scenario, metric) \
                  for trajectory, scenario in zip(trajectories, scenarios) ]
    return Tensor(score_list)
        

def get_single_metric_score(trajectory: Trajectory, scenario: AbstractScenario, metric: MetricBase, time_horizon: float = 8.0) -> float:
    """
    ...
    """
    simulation = get_simulation(scenario)
...
def get_simulation(scenario: AbstractScenario) -> Simulation:
