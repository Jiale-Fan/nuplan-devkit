...
class MapEncoderPts(nn.Module):
    ...
    def __init__(self, d_k, map_attr=3, dropout=0.1):
        ...
        # self.positional_encoding = PositionalEncoding(d_model=self.d_k, dropout=self.dropout, max_len=1000)
    ...
    def forward(self, roads, agents_emb):
        ...
        road_pts_feats = self.road_pts_lin(roads[:, :, :, :self.map_attr]).permute(1, 0, 2, 3).reshape(S, B*P, -1) # (S, B*P, d_k)
